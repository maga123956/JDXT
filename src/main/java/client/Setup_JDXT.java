...
public class Setup_JDXT {
    ...
    private Map<BigInteger, byte[]> cset = new LinkedHashMap<>();//cset
    private Bloom f;
    ...
    public Setup_JDXT(int join_column_num) {
                ...
                if (counter == 0) {
                    ...
                    for (int j = 0; j < key_column; j++) {
                        ...
                        for (Jointuple jointuple : joinArray) {
                            ...
                            cset.put(new BigInteger(xtag), val);
                        }
                    }
                }
                counter++;
    ...
    public void JDXT_update(query query_) throws NoSuchAlgorithmException, InvalidKeyException {
    ...
    public void JDXT_update_batch(ArrayList<query> queryList) throws NoSuchAlgorithmException, InvalidKeyException {
    ...
    public void Store(String text) {
        try {
            FileOutputStream file = new FileOutputStream("data/EDB/JDXT_" + text + ".dat");
            // tset store
            ...
            for (BigInteger xtag : cset.keySet()) {
                ...
                file.write(cset.get(xtag));
            }
            FileChannel channel = file.getChannel();
            System.out.println("JDXT file size is " + (double)channel.size() / 1024.0 / 1024.0 + "MB");
            channel.close();
            file.close();
        } catch (IOException e) {
            throw new RuntimeException(e);
    ...
    public Map<BigInteger, byte[]> getCset() {
        return cset;
    }

    public GGMTree getGgmTree() {
        return ggmTree;
    }
